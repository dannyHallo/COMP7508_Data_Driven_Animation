...
class Database():
    def __init__(self, motions, feature_mapping, selected_feature_names, selected_feature_weights) -> None:
        ...
        # Feature Definition
        self.feature_dims = [feature_mapping[i]
        ...
        for idx, dim_num in enumerate(self.feature_dims):
            ...
            self.feature_weights.extend(
                [selected_feature_weights[idx]]*dim_num)

        self.feature_weights = np.array(self.feature_weights)

        self.joint_name, self.joint_parent, self.joint_offset = motions[
            0].joint_name, motions[0].joint_parent, motions[0].joint_offset
        ...
        self.joint_offset = np.concatenate(
            [[np.zeros((1, 3))], self.joint_offset], axis=0)[:, 0]
        ...
        for bvh_item in motions:
            ...
            for idx, name in enumerate(joint_name):
                ...
                true_motion_data[:, self.joint_channel_offset[true_idx]:self.joint_channel_offset[true_idx+1]
                                 ] = motion_data[:, joint_channel_offset[idx]:joint_channel_offset[idx+1]]
            ...
            tmp_pos, tmp_rot = forward_kinematics_with_channel(
                self.joint_parent, self.joint_channel, self.joint_offset, true_motion_data)
            ...
            fwd_direction = np.array([1, 0, 1]) * \
                sim_rotation.apply(np.array([0, 0, 1]))
            fwd_direction = fwd_direction / \
                np.linalg.norm(fwd_direction, axis=1, keepdims=True)
            ...
            filtered_fwd_direction = filtered_fwd_direction / \
                np.linalg.norm(filtered_fwd_direction, axis=1, keepdims=True)
            ...
            true_motion_data[:, 9:12] = (filtered_sim_rotation.inv() * R.from_euler(
                'XYZ', true_motion_data[:, 9:12], degrees=True)).as_euler('XYZ', degrees=True)
            ...
            features = self.extract_features(
                pos[:, 0], rot[:, 0], pos, rot, vel, avel)
            self.features.append(features)

            # Store the local variables
            local_rot = R.from_euler('XYZ', true_motion_data.reshape(-1, 3),
                                     degrees=True).as_quat().reshape(true_motion_data.shape[0], -1, 4)
            local_rot = align_quat(local_rot, False)
            avel = np.zeros_like(true_motion_data)
            avel[1:] = quat_to_avel(
                local_rot, self.dt).reshape(-1, avel.shape[-1])
            avel[0] = avel[-1]
        ...
        self.local_a_velocities = np.concatenate(
            self.local_a_velocities, axis=0)
        self.local_velocities = np.concatenate(self.local_velocities, axis=0)

        self.frame_num, self.joint_num = self.motion_data.shape[
            0], self.local_a_velocities.shape[1] // 3

        self.features_mean, self.features_std = np.mean(
            self.features, axis=0), np.std(self.features, axis=0)
        self.features_std[self.features_std <= 0.1] = 0.1

        self.normalized_feature = (
            self.features - self.features_mean) / self.features_std
        self.query_tree = cKDTree(
            self.normalized_feature * self.feature_weights)
    ...
    def extract_features(self, root_pos, root_rot, pos, rot, vel, avel):
        ...
        for feature_name in self.feature_names:
            if feature_name == 'lFootPos':
                features.append(self.extract_offset(
                ...
                features.append(self.extract_rotation(
                ...
                features.append(self.extract_vel(
                ...
                features.append(self.extract_vel(
                    root_rot, vel[:, self.joint_name.index('lWrist')]))
            elif feature_name == 'rHandVel':
                features.append(self.extract_vel(
                    root_rot, vel[:, self.joint_name.index('rWrist')]))
                ...
                features.append(self.extract_vel(
                    root_rot, vel[:, self.joint_name.index('lHip')]))
            elif feature_name == 'lHipVel':
                features.append(self.extract_vel(
                    root_rot, vel[:, self.joint_name.index('rHip')]))
                ...
                features.append(self.extract_future_pos(
                    root_pos, root_rot, pos[:, 0]))
    ...
    def normalize_features(self, features):
        normalized_feature = (
            features - self.features_mean) / self.features_std
    ...
    def extract_future_rot(self, root_rot, bone_rot, frames=[20, 40, 60]):
        ...
        for t in frames:
            ...
            direction = (
                rot*R.from_quat(bone_rot[idx])).apply(np.array([0, 0, 1]))
...
class CharacterController:
    def __init__(self, viewer, controller, selected_feature_names, selected_feature_weights):
        ...
        self.db = Database(self.motions, feature_mapping,
                           selected_feature_names, selected_feature_weights)
    ...
    def update_state(self, desired_pos_list, desired_rot_list, desired_vel_list, desired_avel_list, current_gait):
        ...
        if should_search:
            ...
            if 'trajectoryPos2D' in self.db.feature_names:
                ...
                future_pos = np.concatenate([np.array(i).reshape(
                    1, 3) for i in desired_pos_list[1:4]], axis=0)
                future_pos = self.global_bone_rot.inv().apply(
                    future_pos - self.global_bone_pos.reshape(1, 3))
                start_idx = self.db.feature_start_idxs[self.db.feature_names.index(
                    'trajectoryPos2D')]
                end_idx = start_idx + \
                    self.db.feature_dims[self.db.feature_names.index(
                        'trajectoryPos2D')]
                query_feature[start_idx:end_idx] = (
                    future_pos[:, [0, 2]]).flatten()

            if 'trajectoryRot2D' in self.db.feature_names:
                # If you change the 20, 40, 60 setting in the Database class, you should change it here as well.
                # For example, if you change it to 20, 40, 60, 80, you should change the index here to 1:5
                future_rot = np.concatenate([np.array(i).reshape(
                    1, 4) for i in desired_rot_list[1:4]], axis=0)
                future_rot = (self.global_bone_rot.inv(
                ) * R.from_quat(future_rot[:, [1, 2, 3, 0]])).apply(np.array([0, 0, 1]))
                start_idx = self.db.feature_start_idxs[self.db.feature_names.index(
                    'trajectoryRot2D')]
                end_idx = start_idx + \
                    self.db.feature_dims[self.db.feature_names.index(
            ...
            idx = self.db.query_tree.query(
                normalized_query.reshape(1, -1), k=1)[1][0]

            self.search_timer = self.search_time
            if self.pre_motion_data is not None:
                target_rot = self.db.motion_data[idx].reshape(-1, 3).copy()
                target_rot[1] = R.from_quat(
                    desired_rot_list[0]).as_euler('XYZ', degrees=True)
                target_avel = self.db.local_a_velocities[idx].reshape(
                    -1, 3).copy()
                ...
                target_pos[0] = lerp(self.global_bone_pos,
                                     desired_pos_list[0], 0.5)
                target_pos[1] = self.db.motion_data[idx][6:9].copy()
                target_vel = self.db.local_velocities[idx].reshape(-1, 3)[
                    [0, 2]].copy()
        ...
        if self.pre_motion_data is not None:
            ...
            motion_data[3:6] = self.global_bone_rot.as_euler(
                'XYZ', degrees=True)

        self.global_bone_pos = motion_data[0:3]
        self.global_bone_rot = R.from_euler(
            'XYZ', motion_data[3:6], degrees=True)
        self.pre_motion_data = motion_data.reshape(-1, 3)

        joint_translation, joint_orientation = forward_kinematics_with_channel(
            self.db.joint_parent, self.db.joint_channel, self.db.joint_offset, motion_data.reshape(1, -1))
    ...
    def sync_controller_and_character(self, controller, character_state):
        ...
        character_state = (self.db.joint_name,
                           character_state[1], character_state[2])
