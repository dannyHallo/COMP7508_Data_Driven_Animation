...
class CameraCtrl(DirectObject):
    ...
    def _lockMouseInfo(self):
        self._locked_info = (pc.LVector3(self.position),
                             pc.LVector3(self.center), pc.LVector3(self.up))
    ...
    def onUpdate(self, task):
        ...
        if self._mouse_id == 1:
            ...
            rot_y = pc.LMatrix3()
            rot_y.setRotateMat(-mousePosOff.getX() *
                               angleScale, y, pc.CS_yup_right)
...
class SimpleViewer(ShowBase):
    ...
    def load_character(self):
        info = np.load('data/default_skeleton.npy', allow_pickle=True).item()
        ...
        scale[name_idx_map['lHip']] = scale[name_idx_map['rHip']] = [
            thickness, 0.2, thickness]
        scale[name_idx_map['rKnee']] = scale[name_idx_map['lKnee']] = [
            thickness, 0.19, thickness]
        scale[name_idx_map['rAnkle']] = scale[name_idx_map['lAnkle']] = [
            thickness*1.5, thickness, 0.08]
        scale[name_idx_map['rToeJoint']] = scale[name_idx_map['lToeJoint']] = [
            thickness*1.5, thickness*0.6, 0.02]

        scale[name_idx_map['rTorso_Clavicle']] = scale[name_idx_map['lTorso_Clavicle']] = [
            0.05, thickness, thickness]
        scale[name_idx_map['rShoulder']] = scale[name_idx_map['lShoulder']] = [
            0.11, thickness, thickness]
        scale[name_idx_map['rElbow']] = scale[name_idx_map['lElbow']] = [
            0.11, thickness, thickness]
        scale[name_idx_map['rWrist']] = scale[name_idx_map['lWrist']] = [
            0.05, thickness*0.6, thickness*1.2]

        # joint_pos = np.concatenate([body_pos[0:1], joint_pos], axis=0)
        for i in range(len(joint_pos)):
            joint.append(self.create_joint(
                i, joint_pos[i], 'end' in joint_name[i]))
            if i < body_pos.shape[0]:
                body.append(self.create_link(
                    i, body_pos[i], scale[i], rot=body_rot[i] if body_rot is not None else None))
    ...
    def set_joint_orientation_by_name(self, name, quat):
        self.joints[self.name2idx[name]].setQuat(
            self.render, pc.Quat(*quat[..., [3, 0, 1, 2]].tolist()))

    def set_joint_position_orientation(self, link_name, pos, quat):
        if not link_name in self.name2idx:
            return
        self.joints[self.name2idx[link_name]].setPos(
            self.render, *pos.tolist())
        self.joints[self.name2idx[link_name]].setQuat(
            self.render, pc.Quat(*quat[..., [3, 0, 1, 2]].tolist()))
    ...
    def show_pose(self, joint_name_list, joint_positions, joint_orientations):
        ...
        for i in range(length):
            self.set_joint_position_orientation(
                joint_name_list[i], joint_positions[i], joint_orientations[i])
    ...
    def show_rest_pose(self, joint_name, joint_parent, joint_offset):
        ...
        for i in range(length):
            ...
            self.set_joint_position_orientation(
                joint_name[i], joint_positions[i], joint_orientations[i])
